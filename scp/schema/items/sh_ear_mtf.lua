...
ITEM.description = "Тактические наушники"
...
ITEM.functions.EquipOn = {
	...
	OnRun = function(item)
		...
		if (IsValid(client)) and (!client:GetData("equipjmode")) then
			client:ChatPrint("Вы экипировали наушники")
			JmodTakeOnEar(client, item.uniqueID)
		else
			client:ChatPrint("Вы уже экипировали наушники")
		end
...
ITEM.functions.EquipOff = {
	...
	OnRun = function(item)
		...
		if (IsValid(client)) and (client:GetData("equipjmode")) then
			client:ChatPrint("Вы сняли наушники")
			JmodTakeOffEar(client, item.uniqueID)
		else
			client:ChatPrint("Вы уже сняли наушники!")
		end
