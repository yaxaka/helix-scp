ITEM.name = "PNV-10T"
ITEM.model = Model("models/eft_props/gear/nvgs/nvg_alpha_pnv_10t.mdl")
...
ITEM.functions.EquipOn = {
	...
	OnRun = function(item)
		...
		if (IsValid(client)) and (!client:GetData("equipjmodp")) then
			client:ChatPrint("Вы экипировали ПНВ")
			JmodTakeOnPNV(client, item.uniqueID)
		else
			client:ChatPrint("Вы уже экипировали ПНВ")
		end
...
ITEM.functions.EquipOff = {
	...
	OnRun = function(item)
		...
		if (IsValid(client)) and (client:GetData("equipjmodp")) then
			client:ChatPrint("Вы сняли ПНВ")
			JmodTakeOffPNV(client, item.uniqueID)
		else
			client:ChatPrint("Вы уже сняли ПНВ!")
		end
